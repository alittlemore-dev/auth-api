...
from core.account.schemas import TelegramBotSettings
...
class TelegramInternalController(Controller):
    ...
    async def get_settings(
        ...
        with suppress(UserNotFoundError):
            ...
            bot_settings = account.settings.telegram_bots.get(
                telegram_bot_id, TelegramBotSettings()
            )
        return TelegramSettingsResponse(
            available=settings.telegram.is_available_for(telegram_bot_id),
            enabled=bot_settings.enabled,
            notify=bot_settings.notify,
        )
